...
class NanoPlatelet(Configuration):
    def __init__(self, length, width, unit='ang', num_mono_layers=3, 
                 phi=45, pbc=False, balanced=False):
        """
        Parameters
        ----------
        length : float
    ...
    def add_xtal(self, xtal, charges=None, pair_coeffs=None):
        """
        Adds a crystal core.
        ...
        if self.balanced:
            imax = 2*self.num_mono_layers
        else:
            imax = 2*self.num_mono_layers + 1
        ...
        if self.pbc:
            if self.unit == 'ang':
                nx = math.floor(self.length/self.xtal.a)
                ny = math.floor(self.width/self.xtal.b)
            elif self.unit == 'lattice':
                nx = int(self.length); ny = int(self.width)
            ...
            self.simbox[2,1] = (self.num_mono_layers//2)*self.xtal.c
    ...
    def add_ligands(self, ligand_list, ligand_pop_ratio, type_offsets, offset,
                    thickness, packmol_tol=2.0, packmol_sidemax=1.0e3,
                    packmol_path=''):
        """
        ...
        type_offsets : list of tuple
            Each tuple consists of five integers specifying the offsets of atom
        ...
        if thickness < lig_size_max:
            s = input(f"  `thickness` = {thickness} is smaller than the largest ligand"
        ...
        if self.pbc:
            ...
            #Enlarge the current simbox (along z, the non-periodic direction)
    ...
    def solvate(self, solvent, molfrac, type_offsets, boxx, boxy=None,
                boxz=None, dist=0.0, packmol_tol=2.0, packmol_sidemax=1.0e3,
                packmol_path=''):
        """
        ...
        if self.pbc:
            ...
            for i in range(num_components):
                ...
                na_solvent = na_solvent + (comp_pop_above[i]+comp_pop_below[i]) \
                            *solvent[i][0].num_atoms
